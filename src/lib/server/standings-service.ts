...
export type FetchStandingsOptions = {
	includeLiveCurrentRound?: boolean;
};
...
export async function fetchStandingsData(
	...
	const includeLive = options.includeLiveCurrentRound === true;
	...
	const virtualCourtCount = Math.max(...rotations.map((r) => r.courtNumber), courtSizes.length, 1);
	...
	for (let roundNum = 1; roundNum <= currentRound; roundNum++) {
		...
		for (const rotation of roundRotations) {
			...
			const liveCurrent =
				...
				matches.length > 0;
			if (!allMatchesComplete && !liveCurrent) continue;

			const playerIds = rotationPlayerIds(rotation);
			let courtStandings: CourtStandings[];

			if (hasStandingsSnapshot(rotation) && rotation.standingsSnapshot && allMatchesComplete) {
			...
			courtStandings.forEach((standing) => {
				...
				if (stats) {
					...
					stats.roundsPlayed++;
					stats.matchesPlayed += standing.matchCount || requiredMatches;
					...
					if (roundNum === currentRound) {
						...
						if (courtAssignment[standing.playerId]) {
							courtAssignment[standing.playerId].rank = standing.rank;
						}
					}
				}
			});
	...
	const standings = Object.values(playerStats)
		.filter((s) => s.roundsPlayed > 0 || courtAssignment[s.playerId] != null)
		.sort((a, b) => {
			...
			const aRank =
				aHist?.rankOnCourt ?? courtAssignment[a.playerId]?.rank ?? aLastRound?.rankOnCourt;
			const bRank =
				bHist?.rankOnCourt ?? courtAssignment[b.playerId]?.rank ?? bLastRound?.rankOnCourt;
