...
- [] 3p court in the matchups, the single player is shown twice
...
- [] when we enter more than 64 names the system should raise a warning/error, that only max 64 are supported and that at least one player must be removed to proceed
- [] add auto cleanup after test runs
...
- [] extend Non-Standard court Standings tests to actually enter points and see if the players are ranked correctly
...
- [] QR codes in round overview are sometimes not loaded fast enough and don't update, when ready
...
- [] In "tournament.spec.ts/Tournament Integration Tests/complete 2-round tournament with score entry" I see quite some "Failed to fetch" errors in the browser console.
...
- [] score entry doesn't reflect game mode. When we have best of 3 mode we should allow to enter the scores for 3 sets.
