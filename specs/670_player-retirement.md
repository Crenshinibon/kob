...
The remaining unplayed matches for that court must be handled in one of the following two ways.
...
Clicking "Report Injury" opens a selection with two options (the current UI; no restructure option):
...
- **If Substitute (Option A) is selected**: The court view displays the injured player's name as "SUBST" with a "Sub" tag. Remaining matches remain active for score entry. The injury notice shows "A substitute is playing for the injured player."
- **If Cancel & Average (Option B) is selected**: The remaining matches involving the injured player are marked as "Canceled" and disabled in the UI. The organizer only enters scores for any matches that did not involve the injured player.
