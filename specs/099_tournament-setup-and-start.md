...
- The player-count minimum is a **start** condition, not a creation condition. Creating with zero players and adding them one by one, by paste, or by CSV should be normal.

## Goals

1. Tournament lifecycle: **`setup` → `active` → `completed`** (plus the existing `archived` filter on the dashboard).
2. **Create** with name + format + rules and **0–64 players**. No courts, no round 1. Always two steps — there is no "Create & start".
3. **Start** explicitly: requires **4–64** active players; generates courts + round 1; sets `active`, `currentRound = 1`, `startedAt`.
...
6. A tournament in `setup` **is visible to players** who already have a personal URL (`not_started`). Court URLs 404 until start.
...
- A one-tap create-and-start path. Always Create, then Start.
...
│                                                  │
│ [ ▶ Start tournament ]      (needs ≥ 4 players)  │
└──────────────────────────────────────────────────┘
```
...
- **Rounds** and **physical courts** are the same **range sliders** as Create (`RangeSlider`; `setup-num-rounds` min 1 max 10 with current label `"{n} rounds"`, `setup-physical-courts` min 1 max 16 with `"{n} courts"`) and again on the operations view after start (`ops-physical-courts`). Same values as Manage → Tournament (not Rules).
...
This is the second half of today's `createTournamentForm`, extracted into `startTournament()` in `tournament-orchestration.ts` and called from `startTournamentForm` only.
...
| ------------------------------- | -------------------------------- | -------------------------------------------------------------------------- |
| `createTournamentForm`          | `create.remote.ts`               | players optional; always `status = 'setup'`; never starts                  |
...
| `startTournament()`             | `tournament-orchestration.ts`    | called from `startTournamentForm` only                                     |

## i18n Keys (new)

`status_setup`, `dashboard_setup_heading`, `create_players_optional_hint`, `create_rounds_computed_at_start`, `setup_title`, `setup_player_count`, `setup_checked_in_count`, `setup_courts_at_start`, `setup_rounds_duration`, `setup_start_all`, `setup_start_checked_in_only`, `setup_start_removes_note`, `setup_start_button`, `setup_start_needs_players`, `setup_not_started_court_hint`, `player_not_started`, `player_not_started_registered`, `err_tournament_not_in_setup`, `err_tournament_already_started`.
...
3. Create with 16 names → still `setup` (no auto-start) → Start from the panel → round 1. Existing E2E that used the old Create button switch to this two-step path.
...
1. Status name is **`setup`** (not `draft`).
2. **Always two steps** — Create, then Start. No "Create & start".
3. **Checked-in-only start removes** unchecked players (hard delete).
4. **Keep the player textarea** on the creation form.
...
- `src/routes/tournament/create/+page.svelte`, `create.remote.ts` — optional players, Create only (no start)
